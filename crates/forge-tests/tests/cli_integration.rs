...
fn dry_run_creates_no_files() {
  ...
  forge().current_dir(dir.path()).args(["generate", "service", "products", "--dry-run"]).assert().success().stdout(predicate::str::contains("Dry run"));

  // Confirm nothing was actually written
  let entries: Vec<_> = fs::read_dir(dir.path()).unwrap().collect();
  assert!(entries.is_empty(), "dry run must not write any files, but found: {:?}", entries.iter().map(|e| e.as_ref().unwrap().path()).collect::<Vec<_>>())
...
fn dry_run_shows_files_that_would_be_created() {
  ...
  forge().current_dir(dir.path()).args(["generate", "service", "products", "--dry-run"]).assert().success().stdout(predicate::str::contains("products.service.ts")).stdout(predicate::str::contains("CREATE"));
}
...
fn generates_service_files() {
  ...
  forge().current_dir(dir.path()).args(["generate", "service", "products"]).assert().success().stdout(predicate::str::contains("CREATE"));

  let service_file = dir.path().join("src").join("products").join("products.service.ts");
...
fn generate_module_files() {
  ...
  forge().current_dir(dir.path()).args(["generate", "module", "auth"]).assert().success();
...
fn generates_controller_with_correct_route() {
  ...
  let controller_file = dir.path().join("src").join("products").join("products.controller.ts");
...
fn generate_spec_file_by_default() {
  ...
  let spec_file = dir.path().join("src").join("products").join("products.service.spec.ts");

  assert!(spec_file.exists(), "spec file should be generated by default");
}
...
fn no_spec_flag_suppresses_spec_file() {
  ...
  assert!(!spec_file.exists(), "spec file should not be generated when --spec=false");
}
...
fn flat_flag_skips_subdirectory() {
  ...
  let nested_file = dir.path().join("src").join("products").join("products.service.ts");

  assert!(flat_file.exists(), "flat file should exist at {:?}", flat_file);
...
let service_file = dir.path().join("src").join("product-category").join("product-category.service.ts");


assert!(service_file.exists(), "file should use kebab name");

let contents = fs::read_to_string(&service_file).unwrap();
assert!(contents.contains("ProductCategoryService"), "should use PascalCase class name");
assert!(!contents.contains("product-categoryService"), "no kebab casing in class name");
}
...
fn respects_source_root_from_forge_json() {
  ...
  let file = dir.path().join("app").join("products").join("products.service.ts");
...
fn respects_path_override_from_forge_json() {
  ...
  let file = dir.path().join("src/database/entities").join("product").join("product.entity.ts");

  assert!(file.exists(), "entity should go to configured path, no default src/ directory");
}
...
fn cli_spec_flag_overrides_forge_json() {
  ...
  assert!(spec_file.exists(), "CLI --spec=true should override forge.json generateSpec=false");
}
...
assert!(dir.path().join("forge.json").exists(), "forge.json should be created in current directory");

// Verify it's valid JSON
let contents = fs::read_to_string(dir.path().join("forge.json")).unwrap();
let parsed: serde_json::Value = serde_json::from_str(&contents).expect("forge init should write valid JSON");

assert_eq!(parsed["sourceRoot"], "src", "default sourceRoot should be 'src'");
assert_eq!(parsed["generateSpec"], true, "default generateSpec should be true");
