...
pub fn run() -> Result<()> {
  ...
  if config_path.exists() {
    return Err(anyhow!(
      "{} already exists in this directory.\n Delete it first if you want to reinitialize.", style("forge.json").yellow()
    ));
  }
  ...
  let contents = serde_json::to_string_pretty(&default_config).map_err(|e| anyhow!("Failed to seralize config: {e}"))?;
