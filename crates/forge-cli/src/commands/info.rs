...
pub fn run() -> Result<()> {
    ...
    .unwrap_or_else(|_| forge_config::ResolvedConfig {
    ...
    print_info(
        "working dir",
        &style(cwd.display().to_string()).dim().to_string(),
    );

    print_info(
        "project root",
        &style(project_root.display().to_string()).bold().to_string(),
    );
    ...
    if config_found {
        ...
        print_info(
            ...
            &format!(
                "{} {}",
                style(config_path.display().to_string()).bold(),
                style("(found)").green()
            ),
        );
    } else {
    ...
    print_info(
        "source root",
        &style(config.source_root.display().to_string())
            .bold()
            .to_string(),
    );
    ...
    if !config.paths.is_empty() {
        ...
        for (artifact, path) in &sorted_paths {
            print_info(
                artifact,
                &style(path.display().to_string()).bold().to_string(),
            );
        }
    }
    ...
    println!(
        "   {} Shell completion: {}",
        theme::icon_info(),
        style("forge completions <bash|zsh|fish|powershell>").cyan()
    );
